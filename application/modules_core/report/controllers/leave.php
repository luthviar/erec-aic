...
class Leave extends CI_Controller {
    ...
    public function __construct() {
		...
		if ($this->session->userdata('login') == TRUE) {	
			...
			// set sub active
			$this->session->set_userdata('sub_active', 'report_leave');
		}
		else {
    ...
    function index() {
		...
		// set data
		$data['list'] = $this->m_leave->get_report_all();
	...
	function export_to_excel() {
		...
		// set view
		$this->load->view('leave/v_excel',$data);
    }
	...
	function detail_data() {
		...
		if($id == "") {
			...
			if($data['detail'] == null) {
				...
				$this->load->view('../v_header');
				$this->load->view('../v_top');
				$this->load->view('leave/v_detail', $data);
				$this->load->view('../v_bottom');
				$this->load->view('../v_footer');
			}
		}
	...
	function preview_data() {
		...
		if($id == "") {
			...
			if($data['detail'] == null) {
				...
				// set view
				$this->load->view('leave/v_preview', $data);
			}
		}
    }
}