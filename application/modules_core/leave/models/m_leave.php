...
class M_leave extends CI_Model {
	...
	function get_prepared_by_id($id) {
		...
		if ($query->num_rows() > 0) {
			foreach ($query->result() as $row) {
				$leave_prepared = $row->leave_prepared;
			}    

			return $leave_prepared;
		}
		else {
			return 0;
		}	
	...
	function get_report_all() {
		...
		if($this->session->userdata('status') != 11){
			$this->db->where('leave_status', $this->session->userdata('status'));
		}
		
		if($this->session->userdata('from') != ""){
			$this->db->where('leave_date >=', date("Y-m-d", strtotime($this->session->userdata('from'))));
		}
		else{
		...
		if($this->session->userdata('to') != ""){
			$this->db->where('leave_date <=', date("Y-m-d", strtotime($this->session->userdata('to'))));
		}
		else{
			$this->db->where('MONTH(leave_date)', date("m"));
			$this->db->where('YEAR(leave_date)', date("Y"));
