...
class Position extends CI_Controller {
	...
	public $tabel = 'tm_position';
	public $field = 'position_id';
    ...
    public function __construct() {
		...
		if ($this->session->userdata('login') == TRUE) {	
			if($this->session->userdata('authority_id') == 1) { 
				...
				// set sub active
				$this->session->set_userdata('sub_active', 'master_position');
			}
			else {
    ...
    function index() {
		...
		$this->load->view('position/v_list', $data);
	...
	function insert_data() {
		...
		// set array
		$data = array(
			'position_name'	=> handling_characters($name),
		...
		get_redirecting('master/position');
    }
	...
	function edit_data() {
		...
		if($id == "") {
			...
			if($data['detail'] == null) {
				...
				$this->load->view('../v_header');
				$this->load->view('../v_top');
				$this->load->view('position/v_edit', $data);
				$this->load->view('../v_bottom');
				$this->load->view('../v_footer');
			}
		}
